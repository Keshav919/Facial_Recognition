...
def test(model, test_loader, recon_crit, class_crit, kl_weight, name2id, epoch):
    ...
    with torch.no_grad():
        for (batch, data) in tqdm(enumerate(test_loader)):
            ...
            if epoch % 10 == 0:
                plot_preds(img,dec_out, lab, preds, batch, name2id)
...
def plot_preds(img,dec_out, lab, preds, batch, name2id):
...
if __name__ == '__main__':
    ...
    for epoch in range(NUM_EPOCH):
        ...
        print("------- ",epoch,": Train loss: ", train_loss.item(), train_acc.item()," -------")

        test_loss, test_acc = test(model, test_loader, recon_crit, class_crit, kl_weight,name2id, epoch)
